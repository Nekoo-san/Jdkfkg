...
let totalTokensChecked = 0; // Variable zum Verfolgen der Gesamtzahl der geprüften Tokens
...
async function findTarget(imageUrl, place, userId) {
    ...
    while (searchingTarget) {
        if (canceled) {
            console.log('[INFO] Search canceled by user.');
            searchingTarget = false;
        }

        const chosenPlayers = [];
        console.log('[INFO] Preparing to choose 100 player tokens for processing.');
        ...
        if (!chosenPlayers.length) {
            console.log('[INFO] No player tokens left to process, checking if search should continue.');
            await sleep(0);
            if (targetsChecked === playersCount && foundAllServers) {
                console.log('[INFO] All player tokens checked and all servers processed. Ending search.');
                break;
            }
            continue;
        }

        console.log(`[INFO] Sending batch request for ${chosenPlayers.length} player tokens.`);
        await post('thumbnails.roblox.com/v1/batch', JSON.stringify(chosenPlayers)).then(async ({ data: thumbnailsData }) => {
            if (canceled || !searchingTarget) {
                console.log('[INFO] Search has been canceled or completed. Exiting processing loop.');
                return;
            }

            for (const thumbnailData of thumbnailsData) {
                const thumbnails = allThumbnails.get(thumbnailData.requestId) || [];
                console.log(`[INFO] Processing thumbnail data for request ID: ${thumbnailData.requestId}`);

                if (thumbnails.length == 0) {
                    console.log(`[INFO] No existing thumbnails for request ID ${thumbnailData.requestId}, initializing array.`);
                    allThumbnails.set(thumbnailData.requestId, thumbnails);
                }

                targetsChecked += 1;
                totalTokensChecked += 1; // Erhöht die Gesamtanzahl der geprüften Tokens
                console.log(`[INFO] Targets checked so far: ${targetsChecked}`);
                console.log(`[INFO] Total tokens checked: ${totalTokensChecked}`); // Ausgabe der Gesamtanzahl

                if (!thumbnails.includes(thumbnailData.imageUrl)) {
                    console.log(`[INFO] Adding new thumbnail URL to list: ${thumbnailData.imageUrl}`);
                ...
                if (foundTarget) {
                    console.log(`[SUCCESS] Target player found in server ${thumbnailData.requestId}`);
                    ...
                    console.log('[INFO] Fetching friendship tree for the found target.');
    ...
    console.log(`[INFO] Total tokens checked: ${totalTokensChecked}`); // Finale Ausgabe der Gesamtanzahl
